...
def crear_tabla_calls():
    cursor = connection.cursor()
    query = "CREATE TABLE calls(callid INT PRIMARY KEY, agentid INT, customerid INT, pickedup INT, duration INT, productsold INT)"
    try:
        cursor.execute(query)
        print("La tabla ha sido creada exitosamente")
    except psycopg2.errors.DuplicateTable:
...
def crear_tabla_agents():
    cursor = connection.cursor()
    query = "CREATE TABLE agents(agentid INT PRIMARY KEY, name VARCHAR(50) )"
    try:
        cursor.execute(query)
        print("La tabla ha sido creada exitosamente")
    except psycopg2.errors.DuplicateTable:
